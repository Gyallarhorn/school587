...
import { useDeleteExistingPhotoMutation, useUpdateUserMutation } from '../../../redux/api/admin';
...
const UpdateUser = () => {
  ...
  const [deleteImage] = useDeleteExistingPhotoMutation();
  const [updateUser, { isLoading: isLoadingUpdating }] = useUpdateUserMutation();
  ...
  const handleDelete = async () => {
    try {
      ...
      const ImageDeletingResponse = await deleteImage({
        photo: info.photo,
      });

      if (ImageDeletingResponse.error) {
        throw new Error(ImageDeletingResponse.error.data?.message);
      }
      ...
      if (updateResponse.error) {
        throw new Error(updateResponse.error.data?.message);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const photo = existingPhoto === null ? uploadedImagePath : info.photo;
      ...
      if (updatedUserResponse.error) {
        throw new Error(updatedUserResponse.error.data?.message);
      }

      navigate('/admin-panel/checked-users?source=update');
      scrollTo({
        top: 0,
        behavior: 'smooth',
      });
      ...
      toast.success('Выпускник успешно обновлен');
    } catch (err) {
